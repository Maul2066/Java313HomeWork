...
@WebServlet("/CustomizeWorkers")
public class AdminSelectServlet_1 extends HttpServlet{
        private static final String query = "SELECT * FROM worker_bank";
        ...
        protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException,
            ...
            try (Connection connection = DriverManager.getConnection("jdbc:mysql:///banksys", "root", "ExecuteOrder66");
                ...
                out.println("<th style='border: 6px solid black; padding: 10px; background: linear-gradient(90deg, #b02424 0%, #751515 100%);'>Статус</th>");
                ...
                while (resultSet.next()) {
                    ...
                    out.println("<td style='border: 6px solid black; color: red;'>" + resultSet.getString(5) + "</td>");
                    out.println("<td style='border: 6px solid black;'><a style='display: block;  text-align: center; margin: 10px; text-decoration: none; background: linear-gradient(0deg, #f81717 0%, #501313 100%); padding: 10px 0; border-radius: 0 0 6px 6px; color: white; font-weight: bold;' href='editScreen1?id=" + resultSet.getInt(1) + "'>Редактирование</a></td>");
                    out.println("<td style='border: 6px solid black;'><a style='display: block;  text-align: center; margin: 10px; text-decoration: none; background: linear-gradient(0deg, #f81717 0%, #501313 100%); padding: 10px 0; border-radius: 0 0 6px 6px; color: white; font-weight: bold;' href='deleteUrl1?id=" + resultSet.getInt(1) + "'>Удалить</a></td>");
                    out.println("</tr>");
                }
                out.println("</table>");
